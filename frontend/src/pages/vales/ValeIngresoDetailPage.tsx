...
interface ValeIngresoDetailResponse {
  cabecera: ValeIngreso;
  items: ValeItem[];
}
...
export default function ValeIngresoDetailPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["vale-ingreso", id],
    queryFn: () =>
      api
        .get<ValeIngresoDetailResponse>(`/api/vales/ingreso/${id}`)
        .then((r) => r.data),
  });
  ...
  const addItemMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      api.post(`/api/vales/ingreso/${id}/items`, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vale-ingreso", id] });
      setShowAddRow(false);
  ...
  const deleteItemMutation = useMutation({
    mutationFn: (itemId: number) =>
      api.delete(`/api/vales/ingreso/${id}/items/${itemId}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vale-ingreso", id] });
    },
  });
  ...
  const completarMutation = useMutation({
    mutationFn: () => api.post(`/api/vales/ingreso/${id}/completar`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["vale-ingreso", id] });
  ...
  const handleAddItem = () => {
    ...
    if (newItem.fecha_vencimiento)
      payload.fecha_vencimiento = newItem.fecha_vencimiento;
  ...
  const subtotal = items.reduce((sum, it) => sum + it.costo_total, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Button
          variant="ghost"
          size="icon-sm"
          onClick={() => navigate("/vales/ingreso")}
        >
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <SummaryItem
              label="Subtotal"
              value={formatCurrency(cab.subtotal || subtotal)}
            />
            <SummaryItem
              label="IGV (18%)"
              value={formatCurrency(cab.igv || subtotal * 0.18)}
            />
            <SummaryItem
              label="Total"
              value={formatCurrency(
                cab.total || subtotal + subtotal * 0.18
              )}
              highlight
            />
          </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Completar Vale de Ingreso</DialogTitle>
            <DialogDescription>
              Esta seguro de completar este vale? Se actualizara el inventario
              con los {items.length} productos del vale. Esta accion no se puede
              deshacer.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
