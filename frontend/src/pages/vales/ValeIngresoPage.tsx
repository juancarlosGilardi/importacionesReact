...
export default function ValeIngresoPage() {
  ...
  // Conceptos for the form (only when almacen is selected)
  const { data: conceptos } = useQuery({
    queryKey: ["conceptos-ingreso", form.almacen_id],
    queryFn: () =>
      api
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["vales-ingreso", almacenId, estado, search, page],
    queryFn: () =>
      api
        .get<PaginatedResponse<ValeIngreso>>("/api/vales/ingreso", {
          params: {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      api.post("/api/vales/ingreso", payload),
    onSuccess: (res) => {
      const newId = res.data?.id || res.data?.cabecera?.id;
      setShowCreateDialog(false);
      setForm(emptyForm);
      setError("");
      if (newId) navigate(`/vales/ingreso/${newId}`);
    },
    onError: (err: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Vales de Ingreso</h1>
        <Button
          onClick={() => {
            setForm(emptyForm);
            setError("");
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                ...
                >
                  No se encontraron vales de ingreso
                </TableCell>
              </TableRow>
            ) : (
              data.items.map((vale) => (
                <TableRow key={vale.id}>
                  ...
                  <TableCell>{vale.proveedor_nombre || "-"}</TableCell>
                  ...
                  <TableCell>
                    <Button
                      variant="ghost"
                      size="icon-sm"
                      render={<Link to={`/vales/ingreso/${vale.id}`} />}
                    >
      ...
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Nuevo Vale de Ingreso</DialogTitle>
          </DialogHeader>
